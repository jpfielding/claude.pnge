...
|-------|-----------------|
| `pnge-patents:patentsview` | USPTO patent search — full text, claims, assignees, CPC classes |
| `pnge-core:pnge-literature` | Unified search across OpenAlex, CrossRef, USGS Publications Warehouse, and DOE OSTI |
| `pnge-core:datacite-doi` | Research-data DOIs (USGS `10.5066`, OSTI datasets) not in CrossRef |
| `pnge-core:usgs-minerals` | Li commodity context — market drivers for DLE investment |
...
Use `pnge-patents:patentsview` to search USPTO patents:
...
Use `pnge-core:pnge-literature` to find academic publications by the same
...
- Resolve and verify DOIs (CrossRef adapter; DataCite via `pnge-core:datacite-doi` for 10.5066 data DOIs)
...
Use `pnge-core:usgs-minerals` to provide commodity pricing context:
...
This agent is shipped by `pnge-patents`. It references skills in other plugins — install the companions below for full coverage. If a companion is not installed, the agent will still run and will note which pathway is unavailable.

| Companion plugin | Skills referenced |
|---|---|
| `pnge-core` | datacite-doi, pnge-literature, usgs-minerals |
