...
| Skill | Coverage |
|-------|----------|
| `pnge-state-regulatory:wvges-wells` | West Virginia — oil and gas wells, disposal wells |
| `pnge-state-regulatory:padep-wells` | Pennsylvania — permits, completions, production, disposal |
| `pnge-state-regulatory:odnr-wells` | Ohio — permits, Utica/Point Pleasant, injection wells |
| `pnge-state-regulatory:tx-rrc` | Texas — RRC well data, injection permits, H-10 filings |
| `pnge-state-regulatory:nm-ocd` | New Mexico — OCD well permits and injection data |
| `pnge-state-regulatory:nd-dmr` | North Dakota — DMR well data, Bakken injection wells |
| `pnge-state-regulatory:la-sonris` | Louisiana — SONRIS well data, saltwater disposal |
| `pnge-state-regulatory:ar-aogc` | Arkansas — AOGC well data, Smackover disposal |

### Environmental and Seismicity Skills

| Skill | What It Provides |
|-------|-----------------|
| `pnge-federal-data:epa-regulatory` | ECHO compliance (CWA/RCRA/SDW/CAA), Envirofacts (TRI/FRS/NPDES), GHGRP + Subpart W methane. UIC well-level records via state regulators. |
| `pnge-federal-data:usgs-earthquakes` | Earthquake catalog — magnitude, depth, proximity to injection |
| `pnge-well-engineering:well-integrity-barriers` | SCP triage, MIT interpretation, barrier review |
| `pnge-federal-data:wri-aqueduct` | Water stress context for reuse vs. disposal decisions |

### Context Skills

| Skill | What It Provides |
|-------|-----------------|
| `pnge-core:eia-data` | Produced water volumes, production context by state |
| `pnge-core:pnge-literature` | Unified literature — USGS induced seismicity, DOE injection research, peer-reviewed |
...
`pnge-core:eia-data` to establish produced water volumes in the area. This
...
Use `pnge-federal-data:epa-regulatory` (ECHO mode) to cross-reference federal
...
appropriate state regulator skill (`pnge-state-regulatory:tx-rrc`, `pnge-state-regulatory:nm-ocd`,
`pnge-state-regulatory:ok-occ`, `pnge-state-regulatory:co-ecmc`, `pnge-state-regulatory:nd-dmr`, `pnge-state-regulatory:calgem`, or
`pnge-state-regulatory:padep-wells` / `pnge-state-regulatory:odnr-wells` / WVDEP OOG).
...
Use `pnge-federal-data:usgs-earthquakes` to query the earthquake catalog near each
...
Use `pnge-well-engineering:well-integrity-barriers` to assess:
...
Use `pnge-federal-data:epa-regulatory` (ECHO mode) to check compliance and
...
Use `pnge-federal-data:epa-regulatory` (Subpart W mode) to check Subpart W methane
...
This agent is shipped by `pnge-state-regulatory`. It references skills in other plugins — install the companions below for full coverage. If a companion is not installed, the agent will still run and will note which pathway is unavailable.

| Companion plugin | Skills referenced |
|---|---|
| `pnge-core` | eia-data, pnge-literature |
| `pnge-federal-data` | epa-regulatory, usgs-earthquakes, wri-aqueduct |
| `pnge-well-engineering` | well-integrity-barriers |
