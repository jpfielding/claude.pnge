...
description: >
  ...
  OSTI dataset DOIs point at pnge:datacite-doi. Output: unified citation
...
A fifth sibling skill, `pnge:datacite-doi`, handles research-data DOIs
...
`pnge:datacite-doi` when a CrossRef 404 suggests a DataCite DOI.
...
| `10.2172`          | `doe-osti`                | OSTI identifier — may also resolve via `pnge:datacite-doi`    |
| `10.5066`          | `pnge:datacite-doi`       | USGS data releases — NOT in CrossRef, use DataCite            |
...
- Note: USGS data DOIs (`10.5066`) will 404 — point at `pnge:datacite-doi`
...
| "look up DOI 10.5066/P9DSRCZJ"                     | point at `pnge:datacite-doi` (not CrossRef)        |
...
| HTTP 404 on DOI lookup                  | crossref  | Fall back to DataCite via `pnge:datacite-doi` for prefixes 10.5066, 10.2172, 10.25338 |
...
|-----------------------|----------------------------------------------------------------------------|
| `pnge:datacite-doi`   | DOI is `10.5066` (USGS data) or CrossRef returns 404 on a research-data DOI |
| `pnge:patentsview`    | User wants patents alongside peer-reviewed literature (DLE tech scans)     |
| `pnge:netl-edx`       | User needs the underlying dataset cited in an OSTI or USGS report           |
| `pnge:usgs-minerals`  | Commodity summaries complement USGS publication searches on Li/Mg          |
| `pnge:usgs-produced-waters` | Literature on produced-water chemistry pairs with the geochem database |
...
- **DOI coverage**: USGS data releases (`10.5066/*`) and some OSTI
  ...
  `pnge:datacite-doi` to resolve them.
