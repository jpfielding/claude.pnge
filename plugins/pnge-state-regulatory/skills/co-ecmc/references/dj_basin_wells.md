...
Substantial well-to-well variation; use the `pnge-core:usgs-produced-waters`
...
- `pnge-core:usgs-produced-waters` — well-level brine chemistry (Li/Mg/TDS)
- `pnge-federal-data:epa-regulatory` — UIC Class II federal records (UIC intent mode)
- `pnge-federal-data:fracfocus` — Wattenberg stimulation fluid disclosures
- `pnge-well-engineering:frac-design` — engineering analysis of Niobrara/Codell fracs
- `pnge-state-regulatory:regulatory-disposal-analyst` — state-federal disposal overlay
- `pnge-federal-data:netl-carbon-storage` — Class VI context for DJ Basin CCS pilots
- `pnge-engineering-science:aqueous-chemistry-electrochem` — DLE process design for Niobrara
  brines
