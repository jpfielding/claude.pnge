...
- EPA UIC database (via the `pnge:epa-regulatory` skill, UIC intent mode — `UIC_WELL` table)
- The `pnge:regulatory-disposal-analyst` agent for integrated analysis
...
`pnge:netl-carbon-storage` skill for national context and the
`pnge:epa-regulatory` skill (Subpart W mode) for reported GHG inventories.
...
6. **Water chemistry not in production files:** ECMC tracks volumes, not
   ...
   Waters DB (`pnge:usgs-produced-waters`) or operator-reported Form 5A
