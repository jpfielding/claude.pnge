...
- `pnge-core:usgs-produced-waters` for Li/Mg/TDS concentrations by well location
- `pnge-state-regulatory:wvges-wells` for WV well locations with formation targets
- `pnge-federal-data:epa-regulatory` for EPA facility locations (ECHO/FRS); UIC injection wells via state regulator skills
- `pnge-federal-data:boem-offshore` for offshore platform and well locations
- `pnge-federal-data:fracfocus` for hydraulic fracturing job locations
- `pnge-core:eia-data` for state-level production or pricing data (choropleth)
