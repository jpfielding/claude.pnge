...
3. **Limited lithium data in core datasets.** Most published core geochemistry
   ...
   data. For Li concentrations, the `pnge:usgs-produced-waters` skill
...
- Combine with `pnge:usgs-produced-waters` for brine chemistry from the same formations
- Combine with `pnge:wvges-wells` for West Virginia well data overlay
- For Kansas-specific core/log data, see `pnge:kggs-well-logs`
