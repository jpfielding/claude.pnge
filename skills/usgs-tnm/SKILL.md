...
must be inferred from well data (see `pnge:wvges-wells`).
```
...
- Combine elevation data with `pnge:wvges-wells` well locations for terrain context
- Combine with `pnge:blm-mineral-records` for land ownership overlay
