...
`pnge-well-engineering:perforation-design` and any available tracer or DAS data.
...
- Use `pnge-well-engineering:frac-design` for geometry and stress context.
- Use `pnge-well-engineering:perforation-design` for limited-entry and perf friction detail.
- Use `pnge-well-engineering:wellbore-stability` for stress and fracture containment context.
- Use `pnge-well-engineering:production-chemistry` when slurry chemistry or breaker cleanup is implicated.
