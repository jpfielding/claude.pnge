...
*Cross-reference with `pnge-federal-data:epa-regulatory` (UIC Class II well locations) and
`pnge-core:eia-data` (disposal volumes) for injection-seismicity correlation.*
...
- **`pnge-federal-data:epa-regulatory`** — UIC Class II injection well locations and permits.
  Query `UIC_WELL` table (via the UIC intent mode) for disposal wells near earthquake clusters.
- **`pnge-core:eia-data`** — Disposal volume data. Route
  `petroleum/move/imp/data/` and state-level production data can provide
  injection volume context.
- **`pnge-federal-data:fracfocus`** — Hydraulic fracturing disclosures. Correlate
  completion dates with seismicity onset.
- **`pnge-state-regulatory:wvges-wells`** — WV well locations. Cross-reference with
  Appalachian earthquake locations.
- **`pnge-federal-data:boem-offshore`** — Offshore production data for Gulf Coast
