...
- Use `pnge-well-engineering:nodal-analysis-multiphase` for inflow-outflow sensitivity.
- Use `pnge-well-engineering:artificial-lift` for lift-specific tuning.
- Use `pnge-well-engineering:well-integrity-barriers` when annulus or integrity signals appear.
- Use `pnge-well-engineering:rta-production` when the change looks reservoir-driven rather than operational.
