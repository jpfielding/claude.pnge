...
| Skill | Used For |
|-------|---------|
| `pnge-well-engineering:wellbore-stability` | In-situ stress estimation for casing design loads; fracture gradient |
| `pnge-well-engineering:pnge-mechanics` | Thick-walled cylinder burst and collapse verification calculations |
| `pnge-state-regulatory:wvges-wells` | Offset well depths, formations, and casing programs in WV |
| `pnge-core:usgs-produced-waters` | Formation fluid properties (density, corrosivity, H2S content) |
| `pnge-federal-data:epa-regulatory` | ECHO compliance + Envirofacts; UIC Class II (SWD) and Class VI (CO2) well-level records via state regulator skills |
...
- Formation fracture gradient (ppg or psi/ft) — from pnge-well-engineering:wellbore-stability
...
Use `pnge-well-engineering:pnge-mechanics` for detailed thick-walled cylinder calculations
and `pnge-well-engineering:wellbore-stability` to estimate fracture gradient (sets intermediate
...
| Formation pressures not given | Use `pnge-well-engineering:wellbore-stability` to estimate from offset well data; note uncertainty |
...
This agent is shipped by `pnge-well-engineering`. It references skills in other plugins — install the companions below for full coverage. If a companion is not installed, the agent will still run and will note which pathway is unavailable.

| Companion plugin | Skills referenced |
|---|---|
| `pnge-core` | usgs-produced-waters |
| `pnge-federal-data` | epa-regulatory |
