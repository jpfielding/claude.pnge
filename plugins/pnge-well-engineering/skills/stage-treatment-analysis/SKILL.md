...
summary tables. It complements `pnge-well-engineering:completion-diagnostics` by focusing on
...
- Use `pnge-well-engineering:completion-diagnostics` for deeper closure and pressure interpretation.
- Use `pnge-well-engineering:perforation-design` when limited-entry and cluster balance matter.
- Use `pnge-well-engineering:production-chemistry` when slurry chemistry or cleanup affects the stage response.
