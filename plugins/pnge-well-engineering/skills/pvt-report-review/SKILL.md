...
paired with `pnge-well-engineering:petroleum-pvt`, which provides the actual screening
...
- Use `pnge-well-engineering:petroleum-pvt` for screening correlations and gap filling.
- Use `pnge-well-engineering:nodal-analysis-multiphase` for production system use.
- Use `pnge-engineering-science:thermo-eos` if the report includes compositional EOS data.
