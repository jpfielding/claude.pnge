...
Use `pnge-core:usgs-produced-waters` to query geochemistry for the target formation
and location. Gather:
...
Use `pnge-federal-data:wri-aqueduct` to determine baseline water stress for the operating
...
(`pnge-state-regulatory:tx-rrc`, `pnge-state-regulatory:nm-ocd`, `pnge-state-regulatory:ok-occ`, `pnge-state-regulatory:co-ecmc`,
`pnge-state-regulatory:nd-dmr`, `pnge-state-regulatory:calgem`) or WVDEP/PADEP/ODNR for Appalachia. The
...
Use `pnge-federal-data:netl-edx` to search the NEWTS (National Energy-Water Technology
...
Use `pnge-core:pnge-literature` for a unified search across DOE OSTI,
