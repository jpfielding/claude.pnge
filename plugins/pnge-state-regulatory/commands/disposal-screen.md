...
1. **pnge-state-regulatory:wvges-wells** — disposal wells in WV (if Appalachian target)
2. **pnge-state-regulatory:padep-wells** — disposal wells in PA (if Appalachian target)
3. **pnge-state-regulatory:odnr-wells** — injection wells in OH (if Appalachian target)
4. **pnge-state-regulatory:tx-rrc** — disposal wells in TX (if Permian/Eagle Ford target)
5. **pnge-federal-data:epa-regulatory** — ECHO compliance, NPDES, Envirofacts (Note: UIC Class II well-level records are not on Envirofacts — use the relevant state regulator skill above)
6. **pnge-federal-data:usgs-earthquakes** — earthquake catalog within 10-25 km of the target area, 2009-present
7. **pnge-core:eia-data** — produced water volumes for the target area
8. **pnge-federal-data:wri-aqueduct** — water stress context (high stress areas favor reuse over disposal)
