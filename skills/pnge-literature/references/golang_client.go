...
func lookupDOI(raw, mailto string) (Record, error) {
	...
	case strings.HasPrefix(doi, "10.5066/"):
		return Record{}, fmt.Errorf("DOI %q is a DataCite DOI (USGS data release); use pnge:datacite-doi", doi)
	default:
...
func crossrefSingle(doi, mailto string) (Record, error) {
	...
	if code == 404 {
		return Record{}, fmt.Errorf("DOI %q not in Crossref; if prefix is 10.5066 or 10.25338 use pnge:datacite-doi", doi)
	}
	if code != 200 {
